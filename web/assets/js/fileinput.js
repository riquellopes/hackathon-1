...
(function($) {
    ...
    var FileInput = function(element, options) {
        ...
        this.preview_width = options.preview_width;
        this.preview_height = options.preview_height;
                        ...
                        reader.onload = function(theFile) {
                            ...
                            if (isTxt) {
                                ...
                                tempImg.onload = function() {
                                    var MAX_WIDTH = self.preview_width;
                                    var MAX_HEIGHT = self.preview_height;
                                    ...
                                    var diffH = Math.round((tempH - MAX_HEIGHT)/2) * -1;
                                    var diffW = Math.round((tempW - MAX_WIDTH)/2) * -1;

                                    var canvas = document.createElement('canvas');
                                    canvas.width = MAX_WIDTH;
                                    canvas.height = MAX_HEIGHT;
                                    var ctx = canvas.getContext("2d");
                                    ctx.drawImage(this, diffW, diffH, tempW, tempH);
                                    dataURL = canvas.toDataURL("image/jpeg");
                                    
                                    content = '<div class="box-fechar file-preview-frame">\n\n\
                                    <div class="jsFecharFoto  fileinput-remove text-right cor-vermelha">&times;</div><img src="' + dataURL + '" class="img-reponsive" title="' + caption + '" alt="' + caption + '">\n\n\
                                    <input class="form-control" type="text" name="itxDescripton[]" placeholder="Descrição"/></div>';
...
$(function(){
    $("#file-2").fileinput({
        showCaption: false,
        showRemove: false,
        browseLabel: 'Clique para enviar foto',
        showUpload: false,
        browseClass: "",
        fileType: "any",
        preview_width: '210',
